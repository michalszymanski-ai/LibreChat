...
const processAgentFileUpload = async ({ req, res, metadata, sseStream }) => {
  ...
  if (
    ...
    /* Chat attachments also land in the code sandbox when code execution is enabled.
     * Files routed to `execute_code` are skipped: upstream uploads or lazily provisions
     * those on the route the turn actually runs on, so an eager copy would duplicate it. */
    if (messageAttachment && !isImage && effectiveToolResource !== EToolResources.execute_code) {
      try {
